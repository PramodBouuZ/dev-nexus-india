    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <p className="text-center py-10 text-muted-foreground">No messages found.</p>
        ) : (
          messages.map(m => (
            <div key={m.id} className="p-3 rounded-lg border bg-card text-xs flex justify-between items-start gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-bold text-accent">{m.sender_id.slice(0,8)}</span>
                  <span className="text-[10px] text-muted-foreground">in App: {m.application_id.slice(0,8)}</span>
                </div>
                <div className="text-foreground break-words">{m.body || "[Attachment Only]"}</div>
              </div>
              <span className="text-[10px] text-muted-foreground whitespace-nowrap">{new Date(m.created_at).toLocaleTimeString()}</span>
            </div>
          ))
        )}
      </div>
